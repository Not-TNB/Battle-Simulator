...
misc = [Fore.BLUE + i + Fore.RESET for i in ['nothing','disappointment','some water','communism','sand','dirt','a medkit','air???','a twig that snapped when they touched it','a stand arrow']]
...
exinteraction = [[' told ',' to go die'],[' thought that fighting ',' waas futile'],[' showed hentai to ',', who was disgusted'],[' joined ',' in acting weird'],[' caught ',' in 4k'],[' wanted to play ddakji with ',', who quickly refused'],[' tried ','\'(s) homemade cooking'],[' played good cop while ',' played bad cop'],[' was confused on how to play squid game, ',' was confused too']]
deathmsg = [Fore.RED + i + Fore.RESET for i in [' and said a prayer before ending them',' and also slit their throat',' somehow breaking their neck',' and buried them',' thus finishing them off',' and put a bullet through their skull',' and spat on their dead body',' and said "omae wa mou shinderu"',' then teabagged them',' and made a hole through their stomach',' with a stab but didn\'t stop stabbing until they had 69 stab wounds',' and beheaded them']]
...
class BattleField:
  ...
  def progress(self):
    ...
    if random.randint(1, 10) == 1:
      ...
      random.choice([self.eventPlague, self.eventGoblin, self.eventNuclear])()
        ...
        if action <= 2:
          ...
          if f2 == f1:                                           # if the choice is themself,
            ...
            if random.randint(1, 5) == 1:                        # and they roll 1 on a d5,
              self.events += ' and ended up successful\n'        # they die
    ...
    for f1 in self.fighters:
      ...
      if fate <= 2 and len(self.fighters) != 1:
        ...
        if f2 == f1:
          ...
          self.events += f'{f1.Name} smacked {f2.Name} so hard that they died '
...
class Fighter:
  ...
  def heavy(self,other):
    ...
    self.Atk += bonus
    events = Fore.MAGENTA + f'HEAVY ATTACK ({self.Name} ON {other.Name}): ' + Fore.RESET
